...
INPUT_WINDOW = 20
HORIZON = 10  # Predicting only one step ahead
TEST_SIZE = 750
VAL_SIZE = 750
BATCH_SIZE = 64
EPOCHS = 100
...
# ============ SIMPLIFIED MODEL ============
# <... باقی کد شما بدون تغییر تا تعریف DataLoader ها ...>
...
class AdditiveAttention(nn.Module):
    """
    Simple additive attention (Bahdanau-like) over encoder outputs.
    Input: encoder outputs (batch, seq_len, hidden)
    Output: context vector (batch, hidden) and attention weights (batch, seq_len)
    """
    ...
    def forward(self, encoder_outputs):
        ...
        energy = torch.tanh(self.W(encoder_outputs))          # (batch, seq_len, hidden)
...
class CNN_LSTM_Attn(nn.Module):
    def __init__(self, input_size, output_size=1, dropout=0.1, cnn_channels=(64,128), lstm_hidden=128, lstm_layers=2):
        super().__init__()
        ...
        self.conv_block = nn.Sequential(
            nn.Conv1d(in_channels=input_size, out_channels=cnn_channels[0], kernel_size=5, padding="same"),
            nn.LeakyReLU(0.1),
            nn.Conv1d(in_channels=cnn_channels[0], out_channels=cnn_channels[1], kernel_size=3, padding="same"),
            nn.ReLU(),
            nn.Dropout(dropout)
        )

        # --- LSTM for temporal modeling ---
        # LSTM input_size must match cnn_channels[1]
        ...
        # Optional small layer norm on context
    ...
    def forward(self, x):
        ...
        # -> CNN expects (batch, channels=input_size, seq_len)
        x = x.permute(0, 2, 1)  # (batch, input_size, seq_len)

        # CNN feature extraction -> (batch, channels=cnn_channels[1], seq_len)
        x = self.conv_block(x)

        # LSTM wants (batch, seq_len, features)
        x = x.permute(0, 2, 1)  # (batch, seq_len, features=cnn_channels[1])

        # LSTM outputs full sequence (batch, seq_len, hidden)
        lstm_out, _ = self.lstm(x)

        # Attention over LSTM outputs -> context vector (batch, hidden)
        ...
        # Final FC
        out = self.fc(context).squeeze(-1)  # (batch,) if output_size==1

        return out  # if you want attention weights too: return out, attn_weights

# Instantiate model
model = CNN_LSTM_Attn(input_size=X_train.shape[2], output_size=1, dropout=0.1).to(device)

print(f"Model: input={X_train.shape[2]}, output=1")
print(f"Total parameters: {sum(p.numel() for p in model.parameters()):,}")

# Use LR variable you've defined
criterion = nn.MSELoss()
optimizer = torch.optim.Adam(model.parameters(), lr=LR, weight_decay=1e-4)
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=10, factor=0.5, verbose=True)

# ============ ادامه آموزش و ارزیابی همانند قبل ============
# (بقیه کد آموزش، ولیدیشن، ارزیابی، ذخیره‌سازی، و رسم نتایج بدون تغییر می‌ماند)
