...
class SimpleLSTM(nn.Module):
    def __init__(self, input_size, output_size=1):
        ...
        # Simplified LSTM layers - forward only
        self.lstm1 = nn.LSTM(input_size, 32, batch_first=True)  # Reduced from 128
        self.dropout1 = nn.Dropout(0.3)
        
        self.lstm2 = nn.LSTM(32, 16, batch_first=True)  # Reduced from 64
        self.dropout2 = nn.Dropout(0.3)
        
        self.lstm3 = nn.LSTM(16, 8, batch_first=True)  # Reduced from 32
    ...
    def forward(self, x):
        ...
        out = nn.ReLU()(out[:, -1, :])  # Take last output
...
model = SimpleLSTM(X_train.shape[2]).to(device)
...
model_name = f"predictor_{target_col.replace(' ', '_').replace('(', '').replace(')', '')}_horizon_{HORIZON}"
...
time_points = range(len(trues_inv))
ax3.plot(time_points, trues_inv, label='Actual', linewidth=1, alpha=0.8, color='blue')
ax3.plot(time_points, preds_inv, label='Predicted', linewidth=1, alpha=0.8, color='red')
ax3.set_xlabel('Time Sequence')
ax3.set_ylabel('CO Concentration (ppm)')
ax3.set_title(f'Test Period ({len(trues_inv)} samples)')
ax3.legend()
ax3.grid(True, alpha=0.3)

# Plot 4: Last 100 samples
ax4 = plt.subplot(2, 2, 4)
last_100 = min(100, len(trues_inv))
ax4.plot(range(last_100), trues_inv[-last_100:], label='Actual', linewidth=2, color='blue')
ax4.plot(range(last_100), preds_inv[-last_100:], label='Predicted', linewidth=2, color='red')
ax4.set_xlabel('Time Sequence')
ax4.set_ylabel('CO Concentration (ppm)')
ax4.set_title('Last 100 Samples')
ax4.legend()
...
print(f'\nAll files saved with prefix: {model_name}')
print(f'Model and all components saved to: {SAVE_DIR}')
print(f'Simplified training completed for {target_col}!')

# Print deployment instructions
print(f"\n{'='*50}")
print("DEPLOYMENT INSTRUCTIONS:")
print(f"{'='*50}")
print(f"1. Model file: {model_name}.pth")
print(f"2. Feature scaler: {model_name}_x_scaler.pkl")
print(f"3. Target scaler: {model_name}_y_scaler.pkl")
print(f"4. Configuration: {model_name}_config.json")
print(f"5. Target: {target_col}")
print(f"6. Horizon: {HORIZON} hour(s)")
print(f"7. Input window: {INPUT_WINDOW} hours")