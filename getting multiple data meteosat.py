...
def download_product(product, filename, max_retries=3):
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # بعد از دانلود سایز چک می‌کنیم
            if os.path.getsize(filename) == product.size:
                print(f"✅ ذخیره شد: {filename}")
                return True
            else:
                raise ValueError("اندازه فایل ناقص است")
...
for dtstart, dtend in hour_ranges:
    ...
    if products_list:
        ...
        # اگر فایل وجود داشته باشه ولی سایزش ناقص باشه → دوباره دانلود
        if os.path.exists(filename):
            if os.path.getsize(filename) == product.size:
                print(f"⏩ قبلا کامل دانلود شده: {filename}")
                continue
            else:
                print(f"⚠️ فایل ناقص پیدا شد. دوباره دانلود می‌کنم: {filename}")
                os.remove(filename)
